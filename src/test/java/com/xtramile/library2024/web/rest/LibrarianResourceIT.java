...
class LibrarianResourceIT {
    ...
    @Mock
    private LibrarianRepository librarianRepositoryMock;

    @Autowired
    private LibrarianMapper librarianMapper;

    @Mock
    private LibrarianService librarianServiceMock;
    ...
    void getAllLibrariansWithEagerRelationshipsIsEnabled() throws Exception {
        when(librarianServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restLibrarianMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(librarianServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllLibrariansWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(librarianServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restLibrarianMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(librarianRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
